...
class Fun(commands.Cog):
    ...
    async def calculate_blaze(self):
        blaze = sum([1 for _ in range(306) if random() * 100 <= 50])
        return blaze

    async def calculate_pearl(self):
        pearl = sum([1 for _ in range(263) if random() <= (20 / 473)])
        return pearl
    ...
    async def dreamluck(self, ctx):
        """`Test your Minecraft RNG, but in a bot command`"""
        blaze, pearl = await asyncio.gather(self.calculate_blaze(), self.calculate_pearl())
        b = discord.Embed(
